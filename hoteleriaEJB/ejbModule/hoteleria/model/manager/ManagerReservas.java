...
public void guardarFacturaTemporalRecep(Integer IdUsuario,FacReserva facturaCabTmp) throws Exception{
		...
		if(IdUsuario==null || facturaCabTmp.getFacDetalles().size()==0)
			throw new Exception("Debe ingresar en cliente.");
	...
	public List<FacReserva> findAllReservas() {
		String consulta = "select r from FacReserva r ORDER BY r.fechareserva DESC";
		Query q = em.createQuery(consulta, FacReserva.class);
		return q.getResultList();
	}
	
	public void asignarClienteFacturaTmp(FacReserva facturaCabTmp,Integer cedulaCliente) throws Exception{
		System.out.println("LLGAMOS A asignar clinete");
		InvUsuario cliente=null;
		if(cedulaCliente==null||cedulaCliente <=0)
			throw new Exception("Error debe especificar el cliente.");
		try {
			cliente=managerUsuarios.findInvUsuarioById(cedulaCliente);
			if(cliente==null)
